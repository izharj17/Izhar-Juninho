...
class FormulirSM(models.Model):
    ...
    application_number = fields.Char(
        ...
        default=lambda self:
        self.env['ir.sequence'].next_by_code('formulir.sm'))
    ...
    #Kegiatan Hobi dan Pertemanan
    hobi_anak = fields.Char('Apa mainan favorit Anak?')
    teman_hobi = fields.Selection([
        ('ya', 'Ya'),
        ('tidak', 'Tidak')
    ], 'Apakah Anak Memiliki Teman Dekat?')
    kecenderungan = fields.Selection([
        ('sendiri', 'Sendiri'),
        ('sebaya', 'Bersama Orang Lain yang Sebaya'),
        ('orang_dewasa', 'Bersama Orang Lain yang Lebih Dewasa')
    ], 'Kecenderungan Anak Bermain')
    teman_sebaya = fields.Selection([
        ('ya', 'Ya'),
        ('tidak', 'Tidak')
    ], 'Apakah Anak sering bermain dengan teman sebaya?')
    hal_disukai = fields.Text('Apakah hal yang disukai oleh Anak?')
    ...
    metode_aturan = fields.Text('Mohon jelaskan aturan penggunaan gadget yang Anda terapkan pada Anak')
...
class TumbuhKembangSMLine(models.Model):
    _name = "tumbuh.kembang.sm.line"
    _description = "PPDB SM Line"
    
    ppdb_id = fields.Many2one('formulir.sm')
...
class SaudaraKandung(models.Model):
    _name = "saudara.kandung.sm.line"
    _description = "Data Saudara kandung"
    
    saudara_id = fields.Many2one('formulir.sm', readonly=True)
