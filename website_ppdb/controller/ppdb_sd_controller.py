...
class WebsitePPDBSD(http.Controller):

    @http.route('/ppdb/sd', type='http', auth='public', website=True)
    ...
    # @http.route('/api/admission_sd', type='json', auth='public', methods=['POST'], csrf=False)
    # def create_admission_sd(self, **kwargs):
    #     try:
    ...
    @http.route('/api/admission_sd', type='http', auth='public', methods=['POST'], csrf=False, website=True)
    def create_admission_sd(self, **post):
        try:
            # Ambil data dari request
