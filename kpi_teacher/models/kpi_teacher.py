...
class KpiTeacher(models.Model):
    ...
    nilai_total_kpi = fields.Float('Nilai Total', compute='_compute_nilai_total_kpi', store=True, readonly=True)
    ...
    def _compute_nilai_total_kpi(self):
        for record in self:
            ...
            if number_of_questions > 0:
                ...
                record.nilai_total_kpi = min(total_score, 100)  # Cap the score at 100
            else:
                record.nilai_total_kpi = 0
...
class KpiTeacherLine(models.Model):
    ...
    def _onchange_ketuntasan(self):
        if self.kpi_teacher_id:
            self.kpi_teacher_id._compute_nilai_total_kpi()
