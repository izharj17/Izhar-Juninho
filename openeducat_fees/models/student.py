...
class OpStudentFeesDetails(models.Model):
    ...
    amount =  fields.Monetary(compute="_compute_amount",string='Fees Amount',
                                            currency_field='currency_id')
    date = fields.Date('Submit Date')
    product_id = fields.Many2one('product.product', 'Product')
    student_id = fields.Many2one('op.student', 'Student')
    fees_factor = fields.Float("Fees Factor",currency_field='currency_id')
    ...
    #after_discount_amount = fields.Monetary(compute="_compute_discount_amount",string='After Discount Amount',
    #                                        currency_field='currency_id')
    after_discount_amount = fields.Monetary(compute="_compute_discount_amount", string='After Discount Amount',
                                            currency_field='currency_id')
    ...
    def _compute_discount_amount(self):
        for discount in self:
           ...
           # ------- nilai discount langsung diambildari fees_factor -----------
            discount_fees_factor = discount.fees_factor * discount.fees_factor / 100.0
            discount.after_discount_amount = discount.fees_factor

    @api.depends('amount')
    def _compute_amount(self):
        for amount in self:
            amount_fees_factor = amount.fees_factor * amount.fees_factor / 100.0
            amount.amount = amount.fees_factor
        ...
        for records in element_id:
            if records:
                line_values = {'name': records.product_id.name,
                               ...
                               #'price_unit': records.value * self.amount / 10000000000,
                               'price_unit': records.value,
                               'quantity': 1.0,
