...
class OpAttendanceLine(models.Model):
    ...
    present = fields.Boolean(
        'Hadir', default=True, tracking=True)
    excused = fields.Boolean(
        'Izin', tracking=True)
    absent = fields.Boolean('Alfa', tracking=True)
    late = fields.Boolean('Terlambat', tracking=True)
    sick = fields.Boolean('Sakit', tracking=True)
    ...
    batch_id = fields.Many2one(
        ...
        readonly=True)
    remark = fields.Char('Catatan', size=256, tracking=True)
    ...
    def onchange_attendance_type(self):
        if self.attendance_type_id:
            ...
            self.sick = self.attendance_type_id.sick
